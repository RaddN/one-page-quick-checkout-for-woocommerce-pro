...
(function ($) {
    ...
    var RMENUPRO_Add_To_Cart = {
        ...
        init: function () {
            ...
            $(document).on('click', '.rmenupro-ajax-add-to-cart,.single_add_to_cart_button:not(.direct-checkout-button):not(.onepaquc-checkout-btn):not(.product_type_variable),.add_to_cart_button:not(.direct-checkout-button):not(.onepaquc-checkout-btn):not(.product_type_variable)', function(e) {
                if ($(this).closest('form.grouped_form').length) {
                    return;
                }
                RMENUPRO_Add_To_Cart.ajaxAddToCart.call(this, e);
            });
