...
define("RMENUPRO_VERSION", "1.1.9.36");
...
global $onepaqucpro_checkoutformfields, $onepaqucpro_productpageformfields, $onepaqucpro_rcheckoutformfields, $onepaqucpro_string_settings_fields, $onepaqucpro_floating_cart_premium_checkbox_fields, $onepaqucpro_floating_cart_premium_text_fields;
...
$onepaqucpro_string_settings_fields = [
    ...
    "rmenupro_wc_checkout_hover_bg_color",
    "rmenupro_wc_checkout_hover_text_color",
    "rmenupro_wc_checkout_border_radius",
    "rmenupro_wc_checkout_font_size",
    "rmenupro_wc_checkout_width",
    "rmenupro_wc_checkout_custom_width",
...
$onepaqucpro_floating_cart_premium_checkbox_fields = [
    'rmenu_hide_empty_cart_button' => '0',
    'rmenu_floating_cart_show_cart_icon' => '1',
    'rmenu_floating_cart_show_cart_count' => '1',
    'rmenu_floating_cart_show_select_bar' => '1',
    'rmenu_floating_cart_show_item_select' => '1',
    ...
    'rmenu_floating_cart_show_coupon_title' => '1',
    'rmenu_floating_cart_coupon_collapsible' => '0',
    'rmenu_floating_cart_coupon_initially_collapsed' => '0',
...
$onepaqucpro_string_settings_fields = array_values(array_unique(array_merge(
    $onepaqucpro_string_settings_fields,
    array_keys($onepaqucpro_floating_cart_premium_checkbox_fields),
    array_keys($onepaqucpro_floating_cart_premium_text_fields)
)));

function onepaqucpro_can_use_floating_cart_premium_settings()
{
    return function_exists('onepaqucpro_premium_feature') && onepaqucpro_premium_feature();
}

function onepaqucpro_get_floating_cart_element_defaults()
{
    global $onepaqucpro_floating_cart_premium_checkbox_fields;

    return is_array($onepaqucpro_floating_cart_premium_checkbox_fields) ? $onepaqucpro_floating_cart_premium_checkbox_fields : array();
}

function onepaqucpro_get_floating_cart_text_defaults()
{
...
function onepaqucpro_floating_cart_element_enabled($field, $default = '1')
{
    ...
    return get_option($field, $default) === '1';
}

function onepaqucpro_get_floating_cart_text($field, $fallback = '')
{
    if (!onepaqucpro_can_use_floating_cart_premium_settings()) {
        return $fallback;
    }
...
function onepaqucpro_get_floating_cart_selected_suffix()
{
    return onepaqucpro_get_floating_cart_text('txt_Selected', __('selected', 'one-page-quick-checkout-for-woocommerce-pro'));
}

function onepaqucpro_get_floating_cart_checkout_behavior()
{
    $behavior = get_option('rmenu_cart_checkout_behavior', 'direct_checkout');

    if ($behavior !== 'popup_checkout') {
        return 'direct_checkout';
    }

    return onepaqucpro_can_use_floating_cart_premium_settings() ? 'popup_checkout' : 'direct_checkout';
}
...
{
    $popup_iframe_fallback_enabled = onepaqucpro_is_popup_iframe_fallback_enabled();
    ...
    wp_enqueue_style('rmenupro-cart-style', plugin_dir_url(__FILE__) . 'assets/css/rmenu-cart.css', array(), "1.1.9.36");
    if (get_option('onepaqucpro_checkout_layout', 'two_column') === "two_column") {
        wp_enqueue_style('checkout-form-two-column', plugin_dir_url(__FILE__) . 'assets/css/checkout-form-two-column.css', array(), "1.1.9.36");
    }
    $onepaqucpro_cart_script_path = plugin_dir_path(__FILE__) . 'assets/js/cart.js';
    $onepaqucpro_cart_script_version = file_exists($onepaqucpro_cart_script_path) ? (string) filemtime($onepaqucpro_cart_script_path) : '1.1.9.36';
    wp_enqueue_script('rmenupro-cart-script', plugin_dir_url(__FILE__) . 'assets/js/rmenu-cart.js', array('jquery'), "1.1.9.36", true);
...
function onepaqucpro_cart_admin_styles($hook)
{
    ...
    if ($hook === 'toplevel_page_onepaqucpro_cart' || $current_page === 'onepaqucpro_cart_recovery' || $current_page === 'onepaqucpro_cart_recovery_template') {
        ...
        wp_enqueue_style('select2-css', plugin_dir_url(__FILE__) . 'assets/css/select2.min.css', array(), "1.1.9.36");
        wp_enqueue_script('select2-js', plugin_dir_url(__FILE__) . 'assets/js/select2.min.js', array('jquery'), "1.1.9.36", true);
    }
...
function onepaqucpro_is_popup_checkout_enabled()
{
    return get_option('rmenupro_wc_checkout_method', 'direct_checkout') === 'popup_checkout';
}

function onepaqucpro_is_popup_iframe_fallback_enabled()
{
    return (bool) apply_filters('onepaqucpro_enable_popup_iframe_fallback', false);
}
...
function onepaqucpro_request_has_checkout_context_marker($marker_name)
{
    ...
    if (isset($_POST['post_data']) && is_string($_POST['post_data'])) {
        ...
        if (isset($request_data[$marker_name])) {
            $value = strtolower(trim((string) $request_data[$marker_name]));
            return in_array($value, array('1', 'true', 'yes', 'on'), true);
        }
    }
...
function onepaqucpro_should_mount_popup_checkout()
{
    if (!onepaqucpro_is_popup_checkout_enabled()) {
        return false;
    }
    ...
    if (wp_doing_ajax()) {
        return onepaqucpro_request_has_popup_checkout_context();
    }
    ...
    if (onepaqucpro_is_checkout_rendered()) {
        return false;
    }

    if (onepaqucpro_page_has_shortcode('woocommerce_checkout')) {
        return false;
    }
...
function onepaqucpro_maybe_render_checkout_popup()
{
    if (!onepaqucpro_should_mount_popup_checkout()) {
        return;
    }

    onepaqucpro_rmenupro_checkout_popup();
}
add_action('wp_footer', 'onepaqucpro_maybe_render_checkout_popup', 5);
...
function onepaqucpro_rmenupro_checkout_popup($isonepagewidget = false)
{
    ...
    } elseif (!onepaqucpro_is_popup_checkout_enabled() || onepaqucpro_is_checkout_rendered() || onepaqucpro_page_has_shortcode('woocommerce_checkout')) {
        return;
    }
?>
...
function onepaqucpro_custom_quantity_input_on_checkout($html, $cart_item, $cart_item_key)
{
    ...
    if (get_option("rmenupro_quantity_control", "1") === "1") {
        ...
        $new_html .= '<input type="text" class="checkout-qty-input" data-cart-item="' . esc_attr($cart_item_key) . '" value="' . esc_attr($quantity) . '" min="1" step="1" size="4">';
...
function onepaqucpro_force_woocommerce_checkout_mode($is_checkout)
{
    ...
    if ($is_checkout || (is_cart() && !onepaqucpro_is_checkout_rendered())) {
        return $is_checkout;
    }

    if (onepaqucpro_is_checkout_rendered() || onepaqucpro_should_mount_popup_checkout()) {
        return true;
    }
...
function onepaqucpro_is_checkout_rendered($include_single_product_context = true){
    ...
    $has_opcs_shortcode = onepaqucpro_page_has_shortcode('onepaquc_checkout');
...
function onepaqucpro_check_for_plugin_updates($transient, $license_manager)
{
    ...
    $current_version = defined('RMENUPRO_VERSION') ? RMENUPRO_VERSION : '1.1.9.36';
...
add_filter('woocommerce_checkout_update_order_review_expired', 'onepaqucpro_allow_popup_checkout_update_order_review_expired');

function onepaqucpro_maybe_redirect_empty_checkout_cart($should_redirect)
{
    if (isset($_GET['onepaqucpro_add-to-cart'])) {
        return false;
    }
    ...
    if (onepaqucpro_is_popup_iframe_fallback_enabled() && isset($_GET['hide_header_footer']) && wp_unslash($_GET['hide_header_footer']) === '1') {
        return false;
    }

    return $should_redirect;
}
add_filter('woocommerce_checkout_redirect_empty_cart', 'onepaqucpro_maybe_redirect_empty_checkout_cart');
