...
define("RMENUPRO_VERSION", "1.1.6.10");
...
function onepaqucpro_cart_enqueue_scripts()
{
    ...
    wp_enqueue_style('rmenupro-cart-style', plugin_dir_url(__FILE__) . 'assets/css/rmenu-cart.css', array(), "1.1.6.10");
    if (get_option('onepaqucpro_checkout_layout', 'two_column') === "two_column") {
        wp_enqueue_style('checkout-form-two-column', plugin_dir_url(__FILE__) . 'assets/css/checkout-form-two-column.css', array(), "1.1.6.10");
    }
    wp_enqueue_script('rmenupro-cart-script', plugin_dir_url(__FILE__) . 'assets/js/rmenu-cart.js', array('jquery'), "1.1.6.10", true);
    wp_enqueue_script('cart-script', plugin_dir_url(__FILE__) . 'assets/js/cart.js', array('jquery'), "1.1.6.10", true);
    wp_enqueue_script(
...
function onepaqucpro_cart_admin_styles($hook)
{
    if ($hook === 'toplevel_page_onepaqucpro_cart') {
        wp_enqueue_style('onepaqucpro_cart_admin_css', plugin_dir_url(__FILE__) . 'assets/css/admin-style.css', array(), "1.1.6.10");
        wp_enqueue_style('select2-css', plugin_dir_url(__FILE__) . 'assets/css/select2.min.css', array(), "1.1.6.10");
        wp_enqueue_script('select2-js', plugin_dir_url(__FILE__) . 'assets/js/select2.min.js', array('jquery'), "1.1.6.10", true);
    }
    wp_enqueue_style('onepaqucpro_cart_admin_css', plugin_dir_url(__FILE__) . 'assets/css/admin-documentation.css', array(), "1.1.6.10");
    wp_enqueue_script('rmenupro-admin-script', plugin_dir_url(__FILE__) . 'assets/js/admin-documentation.js', array('jquery'), "1.1.6.10", true);
    wp_enqueue_editor();
}
...
{
    if (isset($_POST['onepaqucpro_popup_checkout_context'])) {
        $value = strtolower(trim((string) wp_unslash($_POST['onepaqucpro_popup_checkout_context'])));
    ...
    if (isset($_POST['post_data']) && is_string($_POST['post_data'])) {
        ...
        if (isset($request_data['onepaqucpro_popup_checkout_context'])) {
            $value = strtolower(trim((string) $request_data['onepaqucpro_popup_checkout_context']));
            return in_array($value, array('1', 'true', 'yes', 'on'), true);
        }
    }
...
function onepaqucpro_check_for_plugin_updates($transient, $license_manager)
{
    ...
    $current_version = defined('RMENUPRO_VERSION') ? RMENUPRO_VERSION : '1.1.6.10';
...
{
    if (onepaqucpro_request_has_popup_checkout_context()) {
        return false;
    }
