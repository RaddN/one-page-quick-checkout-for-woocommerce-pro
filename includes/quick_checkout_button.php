    ...
    if ($product_type === 'variable') {
        ...
        if ($product instanceof WC_Product_Variable) {
            ...
            if ($prod_obj instanceof WC_Product_Variable) {
                $variation_id = onepaqucpro_pick_variation_id($prod_obj);
                if (empty($variation_id)) {
                    return false;
                }
            } else {
...
function onepaqucpro_get_product_short_description($product)
{
    ...
    return trim(preg_replace('/\s+/', ' ', $short_description));
}
...
global $onepaqucpro_button_rendered;
$onepaqucpro_button_rendered = false;

function onepaqucpro_add_checkout_button()
{
    global $onepaqucpro_button_rendered;

    if ($onepaqucpro_button_rendered) {
        return;
    }

    if (onepaqucpro_render_checkout_button()) {
        $onepaqucpro_button_rendered = true;
    }
}

function onepaqucpro_add_checkout_button_fallback()
{
    global $onepaqucpro_button_rendered;
...
function onepaqucpro_add_js_fallback()
{
    if (!is_product()) {
        return;
    }
    ...
    global $product;
    if (!$product || !onepaqucpro_should_display_button($product)) {
        return;
    }
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($) {
            ...
            setTimeout(function() {
                ...
                // <?php //else: 
                    ?>
                buttonHtml = '<a class="<?php echo esc_js($button_styling['classes']); ?> onepaquc-checkout-btn" data-product-id="<?php echo esc_js($product_id); ?>" data-product-type="<?php echo esc_js($product_type); ?>" data-title="<?php echo esc_js($product_title); ?>" data-short-description="<?php echo esc_js(onepaqucpro_get_product_short_description($product)); ?>" style="<?php echo esc_js($button_styling['style']); ?>"><?php echo $button_inner; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                                                                                                                                                                                                                                                                                                                                            ?></a>';
                <?php //endif; 
                ?>
...
function onepaqucpro_modify_add_to_cart_button()
{
    ...
    switch ($position) {
        ...
        case 'before_add_to_cart':
            add_action('woocommerce_after_add_to_cart_button', 'onepaqucpro_add_checkout_button', 10);

            // Fallback hooks for different themes
            add_action('wp', function () {
                add_action('woocommerce_single_product_summary', 'onepaqucpro_add_checkout_button_fallback', 35);
                add_action('woocommerce_after_single_product_summary', 'onepaqucpro_add_checkout_button_fallback', 5);
            });
...
if (get_option('rmenupro_add_direct_checkout_button', 1)) {
    ...
    add_action('woocommerce_before_single_product', 'onepaqucpro_modify_add_to_cart_button', 0);
}
...
function onepaqucpro_add_checkout_button_to_add_to_cart_shortcode($link, $product)
{
    ...
    if (isset($onepaqucpro_buy_now_button_in_loop[$context_key])) {
        return $link;
    }

    $onepaqucpro_buy_now_button_in_loop[$context_key] = true;
    ...
    // Final button HTML
    $checkout_button = '<a class="' . esc_attr($button_styling['classes']) . '" data-product-id="' . esc_attr($product_id) . '" data-product-type="' . esc_attr($product_type) . '" data-title="' . esc_html($product_title) . '" data-short-description="' . esc_attr(onepaqucpro_get_product_short_description($product)) . '" style="' . esc_attr($button_styling['style']) . '">' . $button_inner . '</a>';
...
function onepaqucpro_add_checkout_button_after_loop_item()
{
    ...
    echo '<a class="' . esc_attr($button_styles['classes']) . ' onepaqucpro-checkout-btn"'
        ...
        . ' data-short-description="' . esc_attr(onepaqucpro_get_product_short_description($product)) . '"'
...
class onepaqucpro_add_checkout_button_on_archive
{
        ...
        switch ($position) {
            ...
            case 'after_product':
                if (!$this->is_btn_add_hook_works) {
                    add_action('wp_footer', array($this, 'display_overlay_quick_checkout_button_footer'), 110);
                }
                break;
    ...
    public function display_overlay_quick_checkout_button_footer()
    {
        ...
        $allowed_pages = get_option('rmenupro_show_quick_checkout_by_page', ['shop-page', 'category-archives', "tag-archives", 'search', "featured-products", "on-sale", "recent", "widgets", "shortcodes"]);
        $display = false;
...
<?php
    ...
    public function onepaquc_add_checkout_button()
    {
        ...
        // } else {
        // Output the button
        echo '<a class="' . esc_attr($button_styling['classes']) . '" data-product-id="' . esc_attr($product_id) . '" data-product-type="' . esc_attr($product_type) . '" data-title="' . esc_html($product_title) . '" data-short-description="' . esc_attr(onepaqucpro_get_product_short_description($product)) . '" style="' . esc_attr($button_styling['style']) . '">' . $button_inner . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        // }
    }
}
...
function onepaqucpro_button_shortcode_handler($atts = [])
{
    ...
    if ($atts['force'] !== '1' && !onepaqucpro_should_display_button($product)) {
        return '';
    }
    ...
    $styling = onepaqucpro_get_button_styling();
    ...
    $attrs['data-short-description'] = esc_attr(onepaqucpro_get_product_short_description($product));
    if (!empty($variation_id)) {
